...
#Creating a consolidated list of stock details, to facilitate choosing stocks directly via inputing the specific stock ticker
...
#Now that we've chosen our desired company (To run the simulation on), we can proceed with the actual simulation
while True:
    try:
        print()
...
Alternatively, please press [B] to go back: """)
                    ...
                    if date_entered.upper() == "B":
                        ...
                        #Downloads stock price data from the date entered to the most recent date the price data is available for
                        ...
                        #Also downloading data for 5 days prior to the chosen date - To compute moving averages for the simulator
                        ...
                        #Added some verification checks to ensure the format of the entered date is correct
                        ...
                        #Since stock markets are closed on weekends and market holidays, we need to account for non-availability of data on those dates
                        ...
                        #Once a correct date has been entered, the simulation is active
                            ...
                            for i,j in zip(date_list[date_list.index(date_entered):], close[date_list.index(date_entered):]):
                                try:
                                    ...
                                    #Backend computations to ensure the data is made available to plot for the user
                                    ...
                                    #Plotting with matplotlib - This will be visible to the person making use of the simulation                                                                   
                                    ...
                                    #The user is allowed to choose from several possible 'next steps'
                                    action = input("""Press:
[S] - Short
[L] - Long 
...
[N] - no: """).upper()
                                                            ...
                                                            if a == "Y":
                                                                ...
                                                                #This begins a nested loop in which the short position taken up will be navigated date-by-date. This allows a user to focus on navigating and closing out one positon at a time, before they can proceed with their next position on the same/different stock
                                                                for k, q in zip(date_list[date_list.index(i):], close[close.index(j):]): 
                                                                    try:
                                                                        ...
                                                                        #This feature will color the graph based on whether the user is making a profit (green), loss (red), or breakeven (grey)
                                                                        ...
                                                                        #This technique changes the color of the entire line depending on profit/loss/breakeven
                                                                        #For simplicity, I have not included moving averages in this series of graphs, but we can very easily add them here as well
                                                                        plt.style.use('dark_background')
...
[Any key] - Next: """).upper()
                                    ...
                                    #This feature does the exact same thing as shorting, except the money-making happens in the opposite direction (This is the traditional way of stock investing; you make money when the stock price rises)
                                    elif action == "L":
                                        while True:
                                            try:
...
Please do note that you can come back here and open the other tab if you require!: """)
                                                            ...
                                                            #Opens up Investopedia articles on shorting/longing in the user's web browser, allowing them to read up more information on the shorting/longing process
                                                            if follow == "S":
